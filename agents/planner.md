...
- Skip Risks, Test strategy, Rollback (or one line each)
- Skip Open questions unless blocking
...
5. **Rate confidence** using the `confidence-rating-rubric` skill (preloaded). Don't inflate to look decisive — Low is useful information.
...
- No tests: step 1 = "write smoke test"
- No clear convention in codebase → say so, don't invent
