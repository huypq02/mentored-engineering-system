...
This document defines how agents interact with memory. v6 distinguishes between **native auto-memory** (handled by Claude Code) and **deliberate memory** (our `agent_state.md` and `patterns.md`). They have different jobs and don't overlap.
...
|---|---|---|---|
| `agent_state.md` | The deliberate contract — pinned versions, conventions, validated assumptions, decisions, anti-patterns | Months | No |
| `patterns.md` | Meta-learning — confidence patterns, failure patterns, skill gaps over time | Months | Yes (mark resolved/superseded, don't delete) |

Optional third file for multi-session work:

| File | Purpose | When to use |
|---|---|---|
| `session_state.md` | Cross-session bridge for one specific feature | Only for L tasks or multi-day M tasks resumed via `claude -c -r <slug>` |
...
| Agent | agent_state.md | patterns.md | session_state.md (if present) |
|---|---|---|---|
...
| `planner` | Stack, Conventions, Validated assumptions, Known constraints, Decisions, Anti-patterns; in patterns.md: Failure patterns |
...
1. **The `debugger-completion-log.sh` hook** may append a single line to `patterns.md` when debugger finishes a Medium/High-confidence fix. This is automated and bounded.
...
### Pause condition: model switch requested

In addition to stop conditions, agents pause (don't stop, don't continue) when they emit a **Model switch request** per the `model-switch-protocol` skill. They wait for the user to either:
- Switch the model and say "continue" — agent resumes
- Bypass with "stay current tier" / "force <tier>" / "no switches this session" — agent proceeds at current tier
- Re-invoke a different agent variant — original agent's work is complete; the new agent picks up

Pause-on-switch-request applies at any step, not just at task completion.
...
- `session_state.md` should be archived when its feature is complete.
- `patterns.md` is append-only — never delete observations, mark them `[resolved]` or `[superseded]`.
...
When you resume a named session that has a `session_state.md`, mentor reads it on first turn to restore context. This is the only case where `session_state.md` adds value over native auto-memory.
